...
namespace FurnitureFramework.Data
{
	...
	public class TagAttribute : Attribute
	{
		...
		public static List<string> GetRequired(Type type)
		{
			...
			{
				TagAttribute? tag = (TagAttribute?)GetCustomAttribute(field, typeof(TagAttribute));
	...
	/// <summary>
	/// The base class for values inside a Directional Field Dictionary
	/// </summary>
	public class DirField
	{
		...
		// type is passed for the method to work on inherited classes
		{
			...
			if (result.Count == 0) result.Add(DirField.NOROT);
			return result;
		}
	}
	...
	class DirFieldDictConverter<T> : ReadOnlyConverter<DirFieldDict<T>> where T : DirField, new()
	{
				...
				else
				{
					...
					foreach (JProperty property in obj.Properties())
					{
						if (property.Value is JObject dir_obj)
							ReadField(dir_obj, property.Name, ref result);
					}
				}
		...
		static void ReadField(JObject obj, string dir_key, ref DirFieldDict<T> result)
		{
	...
	class DirListConverter<ListT, T> : ReadOnlyConverter<DirListDict<ListT, T>> where ListT : List<T>, new() where T : DirField
	{
		public override DirListDict<ListT, T>? ReadJson(JsonReader reader, Type objectType, DirListDict<ListT, T>? existingValue, bool hasExistingValue, JsonSerializer serializer)
		{
			...
			if (reader.TokenType == JsonToken.StartObject)
			{
				...
				ReadArray(JArray.Load(reader), DirField.NOROT, ref result);

			throw new InvalidDataException($"Could not parse Directional List from {reader.Value} at {reader.Path}.");
		}

		static void ReadArray(JArray array, string direction_key, ref DirListDict<ListT, T> result)
		{
		...
		static void ReadField(JObject obj, string dir_key, ref DirListDict<ListT, T> result)
		{
			DirField.CurrentDirKey = dir_key;
			T? value = obj.ToObject<T>();
			if (value == null || !value.is_valid) return;
